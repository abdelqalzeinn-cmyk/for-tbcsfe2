use std::collections::HashMap;

use bcsfe_derive::{Readable, Writable};

use crate::{blocks::gv_90300::GauntletChapters, stream::HashMapLength};

#[derive(Debug, Clone, Readable, Writable, Default)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[rw(end_assert = 90500)]
pub struct GV90500Block {
    pub collab_gauntlets: GauntletChapters,
    pub u1: bool,
    pub timestamp1: f64,
    pub timestamp2: f64,
    pub u2: i32,
    #[rw(min_gv = 100300)]
    pub u3: Unknown90500_100300,
    #[rw(min_gv = 130700)]
    pub u4: Unknown90500_130700,
    #[rw(min_gv = 140100, with = "HashMapLength<i16, i32, f64>")]
    pub u5: HashMap<i32, f64>,
}

#[derive(Debug, Clone, Readable, Writable, Default)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct Unknown90500_130700 {
    #[rw(with = "HashMapLength<i16, i32, i8>")]
    pub u1: HashMap<i32, i8>,
    #[rw(with = "HashMapLength<i16, i32, f64>")]
    pub u2: HashMap<i32, f64>,
}

#[derive(Debug, Clone, Copy, Readable, Writable, Default)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct Unknown90500_100300 {
    pub u1: i8,
    pub u2: bool,
    pub timestamp1: f64,
    pub timestamp2: f64,
}
